...
class DateInputFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    } else if (text.length == 2 && newValue.text.length > oldValue.text.length) {
      text = "$text/";
    }
...
class _AddCardState extends State<AddCard> {
  ...
  bool _validateInputs() {
    ...
    if (_cardNumberController.text.length < 13 || _cardNumberController.text.length > 19) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text("Write card number:", style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
                const SizedBox(height: 8),
                TextField(
                  controller: _cardNumberController,
                  keyboardType: TextInputType.number,
                  decoration: _inputDecoration(hint: "1111222333444555666", errorText: _cardError),
                ),
                const SizedBox(height: 20),
                Row(
                  children: [
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text("Write CVV:", style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
                      const SizedBox(height: 8),
                      TextField(controller: _cvvController, keyboardType: TextInputType.number, decoration: _inputDecoration(hint: "111", errorText: _cvvError)),
                    ])),
                    const SizedBox(width: 20),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text("Write expiry date:", style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
                      const SizedBox(height: 8),
                      TextField(
                          controller: _expiryController,
                          keyboardType: TextInputType.number,
                          inputFormatters: [FilteringTextInputFormatter.digitsOnly, DateInputFormatter()],
                          decoration: _inputDecoration(hint: "MM/YY", errorText: _expiryError)
                      ),
                    ])),
                  ],
                ),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: () async {
                      ...
                      String? message = await userCtrl.addCard(_cardNumberController.text, _cvvController.text, _expiryController.text, authCtrl.token!);
                    ...
                    child: const Text("Save payment card", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                  ),
                ),
              ],
            ),
  ...
  InputDecoration _inputDecoration({String? hint, String? errorText}) {
    return InputDecoration(
      ...
      // Рамка завжди видима:
      border: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: const BorderSide(color: Colors.black, width: 1.0)),
      enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: const BorderSide(color: Colors.black, width: 1.0)),
      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: const BorderSide(color: Colors.black, width: 2.0)),
      // Підсвітка помилки:
      ...
      focusedErrorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: const BorderSide(color: Colors.red, width: 2.0)),
    );
  }
}